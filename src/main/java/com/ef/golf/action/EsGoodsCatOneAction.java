...
public class EsGoodsCatOneAction extends AbstractService {
    private String both;
    ...
    public Object doService() throws Exception {
        ...
        if (StringUtils.isNotEmpty(both) && both.equals("all")) {
            ...
            map.put("resultList", GoodsCatVoList);
            // map.put("page",page);
        } else {
            int sercondCount = esGoodsCatService.sencondCount(cat_id);
            List<GoodsCatVo> GoodsCatVoList = esGoodsCatService.getSecondGoodsCatVo(maps);
            page.setResultList(GoodsCatVoList);
